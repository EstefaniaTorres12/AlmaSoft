...
export default function HomeAsesor() {
  ...
  const [stats, setStats] = useState({ clientes: 0, planes: 0, productos: 0, afiliados: 0 });
  const [randomProduct, setRandomProduct] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [asesorSession] = useState(() => readAsesorSession());
    ...
    const fetchStats = async () => {
      try {
        const [resClientes, resPlanes, resProductos, resAfiliados] = await Promise.all([
          ...
          authFetch(`${API_URL}/api/client/affiliates/all-for-asesor`),
        ]);
        ...
        {randomProduct && (
          <div className="asesor-info-card">
            ...
            <div style={{ display: "flex", gap: "15px", alignItems: "center", marginTop: "10px" }}>
              <img
                src={randomProduct.producto_imagen || "/img/default/img_default.png"}
                alt={randomProduct.producto_nombre}
                style={{ width: "80px", height: "80px", borderRadius: "12px", objectFit: "cover" }}
                onError={(e) => { e.target.src = "/img/default/img_default.png"; }}
              />
              <div>
