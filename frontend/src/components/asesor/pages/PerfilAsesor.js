...
export default function PerfilAsesor() {
  ...
  const handleSubmit = async (e) => {
    ...
    const fields = [
      ...
      "usuario_documento", "usuario_correo", "usuario_direccion",
    ];

    fields.forEach(field => {
      if (formData[field] !== originalData[field]) payload[field] = formData[field];
    });

    if (formData.usuario_credencial) payload.usuario_credencial = formData.usuario_credencial;
    ...
    try {
      const response = await authFetch(`${API_URL}/api/usuarios/update/${usuarioId}`, {
        method: "PUT",
      ...
      delete updatedUsuario.usuario_credencial;
