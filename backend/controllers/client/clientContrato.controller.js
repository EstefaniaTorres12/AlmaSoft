...
exports.adquirirPlan = (req, res) => {
  ...
  const sqlVerificarCliente = `
    ...
    WHERE c.cliente_id = ? AND u.usuario_id = ?
  `;

  db.query(sqlVerificarCliente, [cliente_id, usuario_id], (err, result) => {
      ...
      db.query(sqlContrato, [valor, cliente_id], (err3, resultContrato) => {
        ...
        db.query(sqlPago, [metodo_pago, contrato_id], (err4) => {
          ...
          db.query(sqlContratoPlan, [contrato_id, plan_id], (err5) => {
            ...
            res.json({ success: true, message: "Plan adquirido correctamente", contrato_id });
          });
        });
      });
    });
