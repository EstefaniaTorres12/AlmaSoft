  ...
  async create(req, res) {
    try {
      ...
      if (!telefono || !usuario_id) {
        return res
          .status(400)
          .json({ message: "telefono y usuario_id son requeridos" });
      }

      // Cliente o Afiliado solo pueden crear para su usuario
      if (
        (userRoles.includes("Cliente") || userRoles.includes("Afiliado")) &&
        Number(usuario_id) !== userId
      ) {
        return res
          .status(403)
          .json({ message: "No puedes agregar teléfonos a otro usuario" });
      }
      ...
      return res
        .status(201)
        .json({ message: "Teléfono agregado correctamente", id });
    } catch (err) {
      console.error(err);
  ...
  async getByUsuario(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "No puedes ver teléfonos de otro usuario" });
      }
  ...
  async update(req, res) {
    try {
      ...
      if (!telefono) {
        return res
          .status(400)
          .json({ message: "Debe ingresar un número de teléfono" });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "No puedes actualizar teléfonos de otro usuario" });
      }
  ...
  async remove(req, res) {
    try {
      ...
      if (userRoles.includes("Asesor")) {
        return res
          .status(403)
          .json({ message: "Asesor no puede eliminar teléfonos" });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "No puedes eliminar teléfonos de otro usuario" });
      }
